#!/usr/bin/env bash
#
# Sherpa-ONNX Bootstrap Script for Emberleaf
# Automatically builds and installs Sherpa-ONNX for KWS and speaker recognition
#
# Usage: bash scripts/install_sherpa_onnx.sh
#
...
build_sherpa() {
    ...
    # CMake 4.x compatibility: Patch root CMakeLists.txt to allow old cmake_minimum_required in dependencies
    # This is a long-term solution that doesn't require patching individual dependency files
    info "Applying CMake 4.x compatibility patch..."
    if ! grep -q "cmake_policy(SET CMP0000 OLD)" CMakeLists.txt; then
        # Insert policy setting after the first cmake_minimum_required line
        sed -i '0,/cmake_minimum_required/a\\n# CMake 4.x compatibility: Allow subdirectories to have older cmake_minimum_required\ncmake_policy(VERSION 3.8)\nif(POLICY CMP0000)\n  cmake_policy(SET CMP0000 OLD)\nendif()' CMakeLists.txt
        success "Applied CMake 4.x compatibility patch"
    else
        info "CMake compatibility patch already applied"
    fi
